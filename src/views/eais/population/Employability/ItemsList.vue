      ...
      >
        <v-container fluid py-3 @click="editElement(item)" class="eais__readerform">
          <v-layout row wrap>
            ...
            <v-flex md3 pt-1>
              <span class="body-1">
                <span v-if="item.data.status !== null">
                  {{ sprRegStatuss[sprRegStatuss.findIndex(x => x.typ_cod == item.data.reg_status)].typ_name }}
                </span>
              </span>
            </v-flex>

            <v-flex md6 pt-1>
              <span class="body-1">Дата рождения: <span class="subheading">{{new Date(item.data.date_borth).toLocaleDateString()}}</span></span>
            </v-flex>
...
export default {
  ...
  methods: {
    ...
    editElement(item) {
      ...
      this.$router.push('employability/' + item.id + '/');
    },
    getListItems(val) {
      console.log();
