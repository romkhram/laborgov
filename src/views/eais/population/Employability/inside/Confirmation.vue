            ...
            <v-card v-for="(item, index) in items" :key="item.id" class="eais__list mb-1">
                <v-container fluid py-3>
                    <v-layout row wrap>
                      ...
                      <v-flex md3>
              					<span class="body-1">
                          <span v-if="item.data.c_client && jobDirRef.find(x => x.c_client === item.data.c_client)">{{jobDirRef.find(x => x.c_client === item.data.c_client).compName}}</span>
                          <span v-else>Организация не найдена</span>
                        </span>
              				</v-flex>
                        ...
                        <v-flex md4>
                          <span class="body-1">
                            <span v-if="item.data.rez_pri">Документ: {{item.data.rez_pri}} </span>
                            <span v-if="item.data.rez_date"> от {{forTime(item.data.rez_date)}}</span>
                          </span>
                        </v-flex>
        ...
        <v-card>
            ...
            <v-container fluid grid-list-xl>
                <v-layout row wrap>
                    ...
                    <v-flex md2 py-1>
                        <date-picker label="с" v-model="editedItem.date_begin"></date-picker>
                    </v-flex>

                    <v-flex md2 py-1>
                        <date-picker label="по" :min="editedItem.date_begin" v-model="editedItem.date_end"></date-picker>
                    </v-flex>
...
export default {
  ...
  data: () => ({
    empSearch: null,
    className: 'job_rez_ref',
  }),
  computed: {
    jobDirRef() {
      var jobDirRef = this.oneItem.linkedData.mpArr_job_dir_ref;
      var jobDirs = [];
      jobDirRef.forEach(function(el, i, arr) {
        jobDirs.push(el.data);
      });
      return jobDirs;
    },
    itemPattern() {
