      ...
      >
        <v-container fluid py-3 @click="editElement(item)">
          <v-layout row wrap>
            ...
            <v-flex md3>
              <span class="body-1">
                <span v-if="item.data.in_date">Дата заявки: {{ forTime(item.data.in_date) }}</span>
              </span>
            </v-flex>

            <v-flex md2>
              <span class="body-1">Численность: {{item.data.k_prof_sum}}</span>
            </v-flex>
...
export default {
  ...
  methods: {
    checkFields() {
      if (this.$store.state.eais.items[0] && this.$store.state.eais.items[0].classCode !== 'cl_zv') {
    ...
    editElement(item) {
      ...
      this.$router.push('conclusion/' + item.id + '/');
    },
    getListItems(val) {
